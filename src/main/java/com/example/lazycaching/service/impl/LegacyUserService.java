  ...
  public List<UserResponse> getUsers() {
    try {
      ...
      if (CollectionUtils.isEmpty(cachedUsers)) {
        ...
        cachedUsers = users.stream().map(this.userMapper::entityToCached)
      ...
      return cachedUsers.stream().map(this.userMapper::cachedToDto).collect(
          Collectors.toList());
    } catch (Exception e) {
      log.error("error during access redis", e);
      log.info("Cache miss!! and fallback with database");
      return this.userRepository.findAll().stream().map(this.userMapper::entityToDto).collect(
          Collectors.toList());
    }
  }
